...
use crate::synth::pi2;
pub use crate::types::synthesis::{
  AmplitudeModParams, FrequencyModParams, Modifiers, ModifiersHolder, ModulationEffect, PhaseModParams,
};
  ...
  pub fn apply(&self, time: f32, y: f32) -> f32 {
    match self {
      ...
      ModulationEffect::Noise(params) => {
        ...
        let mode = ModulationMode::Random {
          seed: params.offset as u64,
        };
...
pub fn get_presets() -> Vec<SimplePreset> {
  vec![
    create_simple_preset!(
      ...
      ModulationEffect::Chorus(PhaseModParams {
        rate: 1.0,
        depth: 0.3,
        offset: 0.1
      })
...
mod test {
  ...
  fn melodic_tremelo() {
    let a = AmplitudeModParams {
      freq: 2.0,
      depth: 0.5,
      offset: 0.1,
    };
    let b = PhaseModParams {
      rate: 1.0,
      depth: 0.3,
      offset: 0.0,
    };
...
pub fn gen_tremelo(
  min_f: f32, max_f: f32, min_d: f32, max_d: f32, min_o: f32, max_o: f32, rng: &mut ThreadRng,
) -> ModulationEffect {
