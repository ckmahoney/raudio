...
use crate::phrasing::contour::{expr_none, Expr};
use crate::phrasing::older_ranger::{Modders, Weight};
use crate::render::blend::blender;
use crate::render::realize::mix_buffers;
use crate::synth::{pi, pi2, pi_2, MFf, NFf, SampleBuffer, MAX_REGISTER, MIN_REGISTER, SR};
use crate::types::render::Span;
use crate::types::synthesis::{Amps, Bp, Freq, Frex, GlideLen, Muls, Note, Phases, Range};
use crate::types::timbre::{Arf, Energy, Mode, Presence, Visibility};
...
pub fn inflect(
  frex: &Frex, at: &ApplyAt, synth: &Elementor, vis: &Visibility, energy: &Energy, presence: &Presence,
) -> SampleBuffer {
  let n_samples = crate::time::samples_of_cycles(at.span.0, at.span.1);
  let druid: Druid = synth
    .iter()
    .map(|(weight, elementor)| (*weight, elementor(frex.2, vis, energy, presence)))
    .collect();

  let mut sigs: Vec<SampleBuffer> = druid
    .iter()
    .map(|(weight, element)| {
      if *weight > 0.0001f32 {
        blender(
...
mod test {
  ...
  fn test_blend_single_element() {
    ...
    let elementor: Elementor = vec![(1f32, nearly_none_enharmonic)];
    let (v, e, p) = test_vep();
...
mod test_modulation {
  ...
  fn test_amplitude_mod() {
    let modifier = applied_modulation::AmplitudeModParams {
      freq: 4.0,
      depth: 1.0,
      offset: 0.0,
    };
