...
impl ModulationMode {
  ...
  pub fn compute(&self, time: f32) -> f32 {
    match self {
      ModulationMode::Sine { freq, depth, offset } => depth * (freq * time + offset).sin(),
      ModulationMode::Linear { slope } => slope * time,
      ModulationMode::Exponential { rate, depth, offset } => depth * (rate * time + offset).exp(),
...
pub fn iterate_modulators(modulators: Vec<ModulationMode>, len: usize) -> Vec<ModulationMode> {
  (0..len).map(|i| modulators[i % modulators.len()].clone()).collect()
}
...
mod test {
  ...
  #[test]
  fn main() {
    let modulator = ModulationMode::Sine {
      freq: 2.0,
      depth: 0.5,
      offset: 0.1,
    };
