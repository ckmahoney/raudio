...
pub fn stem_visible<'render>(arf: &Arf, melody: &'render Melody<Note>) -> Stem<'render> {
  ...
  let mut compound_ratios: Vec<(f32, f32)> = vec![(peak1, 0.6f32), (peak2, 0.9f32), (peak3, 0.3f32)];

  let soids: Soids = soid_fx::concat(&vec![
    compound_ratios.iter().fold(druidic_soids::id(), |soids, (k, gain)| {
  ...
  let expr = (
    vec![visibility_gain(arf.visibility) * visibility_gain(Visibility::Foreground)],
    vec![1f32],
    vec![0f32],
  );
  let feel: Feel = Feel {
