...
) -> Option<(Knob, fn(&Knob, f32, f32, f32, f32, f32) -> f32)> {
...
pub fn driad(arf: &Arf) -> Ely {
  let soids: Soids = match arf.energy {
    Energy::Low => druidic_soids::octave(128f32),
    Energy::Medium => druidic_soids::octave(64f32),
    Energy::High => druidic_soids::octave(32f32),
  };
  let modders: ModifiersHolder = (vec![], vec![], vec![], vec![]);
