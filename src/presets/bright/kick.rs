...
mod test_functional_compressor {
  ...
  use super::*;
  use crate::analysis::{
    sampler::trim_tail_silence,
    tools::{count_energy, dev_audio_asset},
  };
  ...
  fn test_g() {
    ...
    for db in dbs {
      println!(
        "decibels: {} linear amplitude value: {} rms value: {}",
        db,
        db_to_amp(db),
        db_to_amp(db).powi(2i32)
      );
    }
  ...
  fn test_iter_compressor_threshold() {
    ...
    for thresh in thresholds.iter() {
      ...
      match compressor(&samples, compressor_params, None) {
        Ok(result) => {
          let label = format!(
            "hats-compressor-threshold-{}-attack-time-{}-release-time-{}-ratio-{}.wav",
            *thresh, attack_time, release_time, ratio
          );
  ...
  fn test_iter_expander_threshold() {
    ...
    for thresh in thresholds.iter() {
      ...
      match expander(&samples, expander_params, None) {
        Ok(result) => {
          let label = format!(
            "hats-expander-threshold-{}-attack-time-{}-release-time-{}-ratio-{}.wav",
            *thresh, attack_time, release_time, ratio
          );
  ...
  fn test_waveshaping() {
    ...
    let label = format!("hats-expander-then-compressor-threshold-{}.wav", 24);
    render::engrave::samples(SR, &result, &dev_audio_asset(&label));

    let stage_1 = compressor(&samples, compressor_params, None).unwrap();
    let result = expander(&stage_1, expander_params, None).unwrap();
    let label = format!("hats-compressor-then-expander-threshold-{}.wav", 24);
    render::engrave::samples(SR, &result, &dev_audio_asset(&label))
  }
