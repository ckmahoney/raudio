...
pub fn renderable<'render>(conf: &Conf, melody: &'render Melody<Note>, arf: &Arf) -> Renderable2<'render> {
  ...
  let add_saws = |soids:&Soids| -> Soids {
    soid_fx::fmod::sawtooth(&soids, 5)
  };

  let add_squares = |soids:&Soids| -> Soids {
    soid_fx::fmod::square(&soids, 5)
  };

  let richen = |soids:&Soids| -> Soids {
  ...
  let soids = soid_fx::filter_do(&soids, add_squares, |soid| soid.1.log2() > 2f32 && soid.1 % 3f32 == 0f32);
  let soids = soid_fx::merge_soids(&soids);
