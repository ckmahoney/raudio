...
fn noise_pluck(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let a_modu: Option<WOldRangerDeprecateds> = Some(match presence {
    ...
    Presence::Legato => vec![(0.8f32, lifespan::mod_db_pluck), (0.2f32, lifespan::mod_db_fall)],
    Presence::Tenuto => vec![(0.66f32, lifespan::mod_db_fall), (0.33f32, lifespan::mod_db_pluck)],
  });
...
fn melodic_pluck(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let modders: Modders = [Some(vec![(0.65f32, lifespan::mod_snap)]), None, None];
