...
fn melodic_el(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let modders: Modders = [Some(vec![(0.65f32, lifespan::mod_drone)]), None, None];
...
fn layer_sustain(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let expr = (vec![1f32], vec![1f32], vec![0f32]);
  let modders: Modders = [
    Some(vec![(1f32, lifespan::mod_db_pluck)]),
    Some(vec![(1f32, fmod_sweep)]),
    None,
  ];
  Element {
    mode: Mode::Melodic,
...
fn layer_impulse(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let modders: Modders = [Some(vec![(1f32, amod_impulse)]), None, None];
  Element {
    mode: Mode::Bell,
    amps,
    muls,
...
pub fn driad(arf: &Arf) -> Ely {
  ...
  let all_soids = vec![impulse.gain(1f32), sustain.gain(0.95f32)].iter().map(trig::el_to_soid).collect();
