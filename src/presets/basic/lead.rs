...
fn melodic_el(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  ...
  let modders: Modders = [Some(vec![(0.65f32, lifespan::mod_pluck)]), None, None];
...
fn layer_sustain(fund: f32, vis: &Visibility, energy: &Energy, presence: &Presence) -> Element {
  let muls = melodic::amps_square(fund);
  let amps = melodic::amps_square(fund);
  let phss = melodic::amps_square(fund);
  let expr = (vec![1f32], vec![1f32], vec![0f32]);
  let modders: Modders = [None, None, None];
  Element {
    mode: Mode::Melodic,
    amps,
    muls,
...
pub fn driad(arf: &Arf) -> Ely {
  ...
  let all_soids = vec![sustain.gain(1f32)].iter().map(trig::el_to_soid).collect();
