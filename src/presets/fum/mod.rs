...
fn test_attenuate_mod_index_by_vel() {
    for v in 0..=10 {
        ...
        assert!(result >=0f32 && result <= 1.42f32, "Must not exceed the range of (0, 1.42) but actually got {}", result);
    }
}
...
fn test_mod_index_by_moment() {
    ...
    let line:Vec<Note> = (4..15).map(|register| 
        ((1, 1), (register as i8, (1, 0, 3)), amp)
    ).collect();
