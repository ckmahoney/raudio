...
use crate::synth::{pi, pi2, MFf, NFf, SampleBuffer, MF, NF, SR};
use crate::types::render::Span;
use crate::types::synthesis::{
  Bp, Clippers, Direction, Duration, FilterPoint, Freq, Frex, GlideLen, Monae, Mote, Note, Radian, Range, Tone,
};
...
pub fn blender(
  frex: &Frex, expr: &Expr, span: &Span, bp: &Bp, multipliers: &Vec<Freq>, amplifiers: &Vec<Range>,
  phases: &Vec<Radian>, modders: &Modders, thresh: (f32, f32),
) -> SampleBuffer {
  ...
  for j in 0..n_samples {
    ...
    for (i, m) in multipliers.iter().enumerate() {
      ...
      if amplifier > 0f32 {
        ...
        if amp != 0f32 {
          let phase = (frequency * pi2 * t) + phases[i] + pm + mix_or(0f32, &modders[2], k, p, span.1);
          v += amp * phase.sin();
        }
      }
    }
...
pub fn gen_cocktail(n: usize) -> Vec<Mixer> {
  ...
  let weights: Vec<f32> = if n == 1usize {
    ...
    for i in 0..(n - 1) {
      ...
      let next = if i == (n - 2) { rem } else { rng.gen::<f32>() * rem };
      ws.push(next)
    }
...
mod test {
  ...
  fn test_modders_amp() {
    ...
    let amplifiers: Vec<f32> = vec![1f32; multipliers.len()];
    let phases: Vec<f32> = vec![pi2; multipliers.len()];
    let the_modders: Modders = [Some(gen_cocktail(2)), None, None];
    let signal = blender(
      &test_frex(),
      &test_expr(),
      &test_span(),
  ...
  fn test_modders_freq() {
    ...
    let amplifiers: Vec<f32> = vec![1f32; multipliers.len()];
    let phases: Vec<f32> = vec![pi2; multipliers.len()];
    let the_modders: Modders = [None, Some(gen_cocktail(2)), None];
    let signal = blender(
      &test_frex(),
      &test_expr(),
      &test_span(),
  ...
  fn test_modders_phase() {
    ...
    let amplifiers: Vec<f32> = vec![1f32; multipliers.len()];
    let phases: Vec<f32> = vec![pi2; multipliers.len()];
    let the_modders: Modders = [None, None, Some(gen_cocktail(2))];
    let signal = blender(
      &test_frex(),
      &test_expr(),
      &test_span(),
