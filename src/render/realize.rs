...
use crate::types::timbre::{
  AmpContour, AmpLifespan, BandpassFilter, BaseOsc, Energy, FilterMode, Phrasing, Presence, Sound2, Timeframe,
};
...
fn find_global_max_amplitude(channels: &[Vec<f32>]) -> f32 {
  channels.iter().flat_map(|channel| channel.iter()).map(|&sample| sample.abs()).fold(0.0, f32::max)
}
