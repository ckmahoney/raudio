...
//!
//! This exploratory module was a precursor to the current additive model.
...
use crate::phrasing::contour::{sample, Expr, Position};
use crate::phrasing::older_ranger::{example_options, mix, Mixer, Modders, OldRangerDeprecated, WOldRangerDeprecateds};
...
use crate::types::synthesis::{
  Bp, Direction, Duration, FilterPoint, Freq, Frex, GlideLen, Monae, Mote, Note, Radian, Range, Tone,
};
use crate::types::timbre::{BandpassFilter, BaseOsc, Energy, FilterMode, Phrasing, Presence, Sound, Timeframe};
...
pub fn ninj<'render>(
  Ctx {
    freq,
    span,
    thresh: &(gate_thresh, clip_thresh),
  }: &'render Ctx,
  FeelingHolder {
    bp,
    expr,
  ...
  let wets: Vec<SampleBuffer> = reverbs.iter().map(|params| convolution::of(&sig, &params)).collect();
...
pub fn ninja<'render>(
  span: &Span, thresh: (f32, f32), freq: f32, expr: &Expr, bp: &Bp, dressing: &Dressing, modifiers: Modifiers<'render>,
) -> SampleBuffer {
...
mod test {
  ...
  use super::*;
  use crate::druid::applied_modulation::{AmplitudeModParams, FrequencyModParams, ModulationEffect, PhaseModParams};
  ...
  fn test_ninja_tremelo() {
    ...
    let modifiers: Modifiers = (&vec![effect], &vec![], &vec![], &vec![]);
    let signal = ninja(&span, thresh, freq, &expr, &bp, &dressing, modifiers);
    write_test_asset(&signal, &test_name)
  }
  ...
  fn test_ninja_vibrato() {
    ...
    let modifiers: Modifiers = (&vec![], &vec![effect], &vec![], &vec![]);
    let signal = ninja(&span, thresh, freq, &expr, &bp, &dressing, modifiers);
    write_test_asset(&signal, &test_name)
  }
  ...
  fn feeling_lead(freq: f32, amp: f32) -> FeelingHolder {
    ...
    FeelingHolder {
      expr: (
        vec![amp, amp / 4f32, amp / 10f32, amp / 10f32, 0f32],
        vec![1f32],
        vec![0f32],
      ),
  ...
  fn test_ninja_xfiles_lead() {
    ...
    for syn_midi in line {
      ...
      let thresh: (f32, f32) = (0f32, 1f32);
      let expr = (
        vec![xform_freq::velocity_to_amplitude(syn_midi.2)],
        vec![1f32],
        vec![0f32],
      );
      let bp = (vec![MFf], vec![NFf]);
      ...
      let modifiers: Modifiers = (&vec![], &vec![], &vec![effect], &vec![]);
  ...
  fn test_ninja_xfiles_with_delay() {
    ...
    let rs: Vec<(Vec<Vec<(f32, i32, i8)>>, fn(f32, f32) -> FeelingHolder)> =
      vec![(melody1, feeling_lead), (melody2, feeling_chords)];

    let ds: Vec<DelayParams> = vec![DelayParams {
      mix: 0.5f32,
      gain: 0.99,
      len_seconds: 0.15f32,
      n_echoes: 5,
    ...
    match render::pad_and_mix_buffers(channels) {
      Ok(signal) => write_test_asset(&signal, &test_name),
      Err(msg) => {
  ...
  fn test_ninja_xfiles_reverb() {
    ...
    let stems: Vec<(Vec<Vec<(f32, i32, i8)>>, fn(f32, f32) -> FeelingHolder)> =
      vec![(melody1, feeling_lead), (melody2, feeling_chords)];
    ...
    for (index, (midi_melody, synth_gen)) in stems.iter().enumerate() {
      let line_buffs: Vec<SampleBuffer> = midi_melody
        .iter()
        .map(|line| {
          let mut channel_samples: Vec<SampleBuffer> = Vec::new();
