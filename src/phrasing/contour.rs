...
use crate::types::synthesis::{Freq, Radian, Range};
...
pub fn sample(contour: &AmpModulation, p: f32) -> f32 {
  ...
  let index = ((p * contour.len() as f32) as usize).min(contour.len() - 1).max(0);
  ...
  let interp = (1f32 - pos) * contour[index] + (pos * contour[n]);
  interp
}
...
mod test_gen_contour {
  ...
  fn test_contour_fade() {
    ...
    assert_eq!(
      amp_mod.len(),
      n_samples,
      "Must produce a contour of the requested duration in cycles"
    );
    ...
    assert_eq!(
      amp_mod.len(),
      n_samples,
      "Must produce a reversed contour of the requested duration in cycles"
    );
  ...
  fn test_contour_surge() {
    ...
    assert_eq!(
      amp_mod[0],
      0f32,
      "Polynomial surge must start at 1. Found {:#?}",
      &amp_mod[0..10]
    );
    ...
    assert!(
      (1f32 - f) <= err_margin,
      "Must start near 1 when using reverse linear surge"
    );
    let v: f32 = amp_mod[amp_mod.len() - 1];
...
mod test_apply_contour {
  ...
  fn test_apply_contour() {
    ...
    let contour: AmpModulation = vec![
      0f32, 0.1f32, 0.2f32, 0.3f32, 0.4f32, 0.5f32, 0.6f32, 0.7f32, 0.8f32, 0.9f32,
    ];

    apply_contour(&mut signal, &contour);
    assert_eq!(
      signal, contour,
      "When provided a unit input vector, must shape the input to contour exactly"
    )
  }
  ...
  fn test_apply_contour_inner_interpolation() {
    let mut signal: SampleBuffer = vec![1.0f32; 10];
    let contour: AmpModulation = vec![0f32, 1f32];
    let expected: SampleBuffer = vec![0f32, 0.2f32, 0.4f32, 0.6f32, 0.8f32, 1f32, 1f32, 1f32, 1f32, 1f32];

    apply_contour(&mut signal, &contour);
    assert_eq!(
      signal, expected,
      "When provided a contour shorter than the input vector, must use linear interpolation to extrapolate values"
    )
  }
  ...
  fn test_apply_contour_outter_interpolation() {
    let mut signal: SampleBuffer = vec![1.0f32; 10];
    let contour: AmpModulation = vec![
      0.00f32, 0.05f32, 0.10f32, 0.15f32, 0.20f32, 0.25f32, 0.30f32, 0.35f32, 0.40f32, 0.45f32, 0.50f32, 0.55f32,
      0.60f32, 0.65f32, 0.70f32, 0.75f32, 0.80f32, 0.85f32, 0.90f32, 0.95f32,
    ];
    ...
    assert_eq!(
      signal, expected,
      "When provided a contour longer than the input vector, must use the nearest relative point in values"
    )
  }
}
