...
use crate::synth::{e, epi, pi, pi2, MFf, NFf, SR};
use crate::types::synthesis::SampleBuffer;
use crate::types::timbre::{Arf, Energy, MicroLifespan, Mode, Presence, Visibility};
...
pub static micros: [MicroLifespan; 3] = [MicroLifespan::Pop, MicroLifespan::Chiff, MicroLifespan::Click];
...
pub fn modders_click() -> crate::phrasing::older_ranger::Modders {
  [None, Some(vec![(1f32, amp_click)]), Some(vec![(1f32, mod_phase_micro)])]
}

pub fn modders_pop() -> crate::phrasing::older_ranger::Modders {
  let mut s = rand::thread_rng();

  [None, Some(vec![(1f32, amp_pop)]), None]
}
...
mod test {
  ...
  fn assert_microtransient(micro: &MicroLifespan, mod_signal: &Vec<f32>) {
    let max_sample_num_greater_than_0: usize =
      1 + (time::samples_of_dur(1f32, 1f32) as f32 * threshold_x_cutoff) as usize;
    let max_index = max_sample_num_greater_than_0 + max_sample_num_greater_than_0 / 2;

    for (i, y) in mod_signal.iter().enumerate() {
      assert!(
        false == y.is_nan(),
        "Modulation micro {:#?} must only produce numeric values. Got NAN at index {}",
        micro,
        i
      );
      assert!(
        *y <= 1f32,
        "Modulation micro {:#?} must not produce any values above 1. Found {} at index {}",
        micro,
        y,
        i
      );
      assert!(
        *y >= -1f32,
        "Modulation micro {:#?} must not produce any values below -1. Found {} at index {}",
        micro,
        y,
        i
      );
    ...
    assert!(
      early_rms > 0.00001f32,
      "Early RMS must be easily audible for {:#?}. Got actual value of {}",
      micro,
      early_rms
    );
    ...
    assert!(
      rms < 1f32,
      "Modulation micro {:#?} must produce an RMS value less than 1. Got {}",
      micro,
      rms
    );
    assert!(
      rms > 0f32,
      "Modulation micro {:#?} must produce an RMS value greater than 0. Got {}",
      micro,
      rms
    );
  }
  ...
  mod test_integration {
    ...
    use crate::druid::{inflect, modders_none, test_frex, test_vep, ApplyAt, Element, Elementor};
    use crate::files;
    ...
    fn test_blend_single_element_chiff() {
      ...
      let elementor: Elementor = vec![(1f32, nearly_none_chiff)];
    ...
    fn test_blend_single_element_click() {
      ...
      let (vis, en, pre) = test_vep();
      let elementor: Elementor = vec![(1f32, nearly_none_click)];
    ...
    fn test_blend_single_element_pop() {
      ...
      let (vis, en, pre) = test_vep();
      let elementor: Elementor = vec![(1f32, nearly_none_pop)];
