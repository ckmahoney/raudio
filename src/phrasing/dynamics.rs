...
fn generate_offset_triangle_wave(
  offset_x_samples: f32, width_samples: f32, min_y: f32, max_y: f32, midpoint_y_progress: f32, n_samples: usize,
) -> Vec<f32> {
...
fn generate_layers(n_layers: usize, n_samples: usize, greatest_reduction: f32, least_reduction: f32) -> Vec<Vec<f32>> {
  ...
  for i in 1..=n_layers {
    ...
    let samples = generate_offset_triangle_wave(
      offset_x_samples,
      layer_width,
      min_y,
      max_y,
...
fn sum_and_dynamic_normalize_layers(gain: f32, layers: &[Vec<f32>]) -> Vec<f32> {
  ...
  let max_values: Vec<f32> =
    layers.iter().map(|layer| *layer.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()).collect();
  ...
  result
    .iter_mut()
    .for_each(|value| *value = gain * (*value / result_max) * peak_range + min_peak_value);
