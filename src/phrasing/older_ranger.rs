...
pub static example_options:[OldRangerDeprecated; 3] = [
    a,
    b,
    c
];
...
pub fn mix(k:usize, x:f32, d:f32, mixers:&WOldRangerDeprecateds) -> f32 {
    ...
    if weight > 1f32 {
        panic!("Cannot mix OldRangerDeprecateds whose total weight is more than 1. Got {}", weight)
    };

    mixers.iter().fold(0f32, |y, (w, OldRangerDeprecated)| y + (w * OldRangerDeprecated(k, x, d)))
}
...
mod test {
        ...
        for (i, OldRangerDeprecated) in (&example_options).iter().enumerate() {
            for k in MONICS {
                ...
                for x in DOMAIN {
                    ...
                    assert!(y >= min, "OldRangerDeprecated {} must not produce values below {}", i, min);
                    assert!(y <= max, "OldRangerDeprecated {} must not produce values above {}", i, max);
                }
                assert!(has_value, "OldRangerDeprecated {} must not be 0 valued over its domain", i);
                assert!(not_one, "OldRangerDeprecated {} must not be 1 valued over its domain", i);
            }
        }
    }
    ...
    #[test]
    fn test_mix() {
        let mixers:Vec<Mixer> = (&example_options).iter().map(|OldRangerDeprecated| (1f32/example_options.len() as f32, *OldRangerDeprecated)).collect();
        for k in MONICS {
            let mut has_value = false;
            let mut not_one = false;
            for x in DOMAIN {
