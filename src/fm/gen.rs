...
use crate::{Note,Mode,Role,Energy,Presence,Conf,Arf,Melody};
...
impl OperatorSpec {
    ...
    pub fn to_operator(&self, parent_freq: f32) -> Operator {
        ...
        let mod_index_env_mul = build_envelope_from_lifetime(
            self.lifetime_shape.clone(),
            self.lifetime_value
        );
...
pub fn random_operator_structure(depth: usize, max_depth: usize) -> OperatorSpec {
    ...
    let lf_val = 1.0;
    // Random shape
    let shapes = [LifetimeSpec::Burp, LifetimeSpec::Pluck, LifetimeSpec::Fall, LifetimeSpec::Rise];
...
fn calculate_depth(op: &Operator) -> usize {
    ...
    op.modulators.iter().filter_map(|modulator| {
        if let ModulationSource::Operator(inner_op) = modulator {
...
fn test_my_rendered_synth() {
    ...
    for (i, note) in melody[0].iter().enumerate() {
        ...
        while let None = get_remaining_range(&op, 0f32, 0f32, crate::synth::MFf, NFf) {
            eprintln!(
                "Note {} @{}Hz: Bandwidth exceeded. Pruning operator.",
                i, freq
            );
        ...
        println!(
            "Note {} @{}Hz: Operator valid. Rendering...",
            i, freq
        );
