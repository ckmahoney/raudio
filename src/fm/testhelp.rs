...
pub fn random_knob() -> Knob {
  ...
  pub fn random_modulation_function() -> Ranger {
    let mut rng = rand::thread_rng();
    let modulation_functions: Vec<fn(&Knob, f32, f32, f32, f32, f32) -> f32> =
      vec![ranger::amod_fadeout, 
  ...
  pub fn random_modulator(cps: f32, n_cycles:f32, fund: f32, z_index: usize) -> Operator {
    ...
    let mod_func: Ranger = if rng.gen::<f32>() < 0.5f32 {
        random_modulation_function() } else {
    ...
    let mod_index_env = if rng.gen::<f32>() < 0.5f32 {
        vec![
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles/3f32),
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles/3f32),
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles/3f32),
        ].concat()
    } else {
    ...
    let modulators = if z_index > 0 {
        vec![ModulationSource::Operator(random_modulator(cps, n_cycles, fund, z_index -1))]
    } else {
        vec![]
    };
    // Construct modulator
    Operator {
      ...
      mod_index_env_mul: Envelope::from_samples(&mod_index_env), // Envelope for modulation index
...
pub fn random_modulator_with_envelope(cps: f32, n_cycles: f32, base_freq: f32, depth: usize) -> Operator {
    ...
    let mod_index_env = vec![
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles / 3.0),
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles / 3.0),
        ranger::eval_knob_mod(random_modulation_function(), &random_knob(), cps, mod_freq, n_cycles / 3.0),
    ]
    .concat();
    ...
    let modulators = if depth > 0 {
        vec![ModulationSource::Operator(random_modulator_with_envelope(cps, n_cycles, base_freq, depth - 1))]
    } else {
        vec![]
    };
    ...
    Operator {
        ...
        modulation_index: rng.gen_range(0.5..2.0), // Random modulation index
        modulators,
        mod_index_env_mul: Envelope::from_samples(&mod_index_env), // Envelope for dynamic modulation
        mod_index_env_sum: Envelope::unit_sum(),
        mod_index_mul: None,
        mod_index_sum: None,
        mod_freq_env_mul: Envelope::unit_mul(),
