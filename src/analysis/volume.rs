...
pub fn is_monotonically_increasing_rms(signal: &SampleBuffer, sample_rate: usize) -> bool {
  ...
  for t in 0..num_buckets {
    ...
    if !bucket.is_empty() {
      // Check to ensure non-empty bucket
      let r = rms(&bucket);
...
mod test {
  ...
  fn test_increase_happy_path_valid() {
    ...
    let signal: SampleBuffer = (0..n_samples)
      .enumerate()
      .map(|(index, _)| i.powi(2i32 + (2i32 * index as i32)).re * index as f32 / n_samples as f32)
      .collect();

    let result = is_monotonically_increasing_rms(&signal, signal.len() / 10);
    assert!(
      result,
      "Must be able to measure that a constantly growing signal is growing"
    )
  }
  ...
  fn test_increase_happy_path_invalid() {
    ...
    let signal: SampleBuffer = (0..n_samples)
      .enumerate()
      .map(|(index, _)| i.powi(2i32 + (2i32 * index as i32)).re * (n_samples - index) as f32 / n_samples as f32)
      .collect();
