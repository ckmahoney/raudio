...
/// Convert from generic Monae to 12-ET representation of a tone
...
mod test {
  ...
  fn test_check_notes() {
    ...
    let expected: Vec<i8> = vec![
      43, 43, 45, 43, 48, 47, 43, 43, 45, 43, 50, 48, 43, 43, 55, 52, 48, 47, 45, 53, 53, 52, 48, 50, 48,
    ];
  ...
  fn view_early_monics() {
    ...
    println!(
      "Fit {:?}",
      (0i32..=110).into_iter().map(|x| (3f32 / 2f32).powi(x)).collect::<Vec<f32>>()
    );
