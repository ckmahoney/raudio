...
pub fn groove_optimizer(
  input_signal: AudioFormat, transient_emphasis: f32, dynamic_range: f32, rhythmic_threshold: f32, time_constant: f32,
) -> AudioFormat {
  ...
  match input_signal {
    ...
    AudioFormat::Stereo(left, right) => {
      ...
      let processed_left = process_channel(
        left,
        transient_emphasis,
        dynamic_range,
        left_threshold,
      ...
      let processed_right = process_channel(
        right,
        transient_emphasis,
        dynamic_range,
        right_threshold,
...
fn process_channel(
  samples: Vec<f32>, transient_emphasis: f32, dynamic_range: f32, threshold: f32, attack_time: f32, release_time: f32,
) -> Vec<f32> {
