...
fn detect_transient(samples: &SampleBuffer, window_length: usize, threshold: f32) -> Vec<usize> {
    ...
    for window_start in (0..samples.len()).step_by(window_length) {
        ...
        let current_rms: f32 = (window.iter().map(|&sample| sample.powi(2) as f32).sum::<f32>() / window.len() as f32).sqrt();
...
mod tests {
    ...
    fn test_transient_in_center() {
        ...
        assert_eq!(detected_transients, vec![40], "Transient should be detected at sample index 40");
    }
    ...
    fn test_transient_at_two_thirds() {
        ...
        assert_eq!(detected_transients, vec![90], "Transient should be detected at sample index 90");
    }
}
