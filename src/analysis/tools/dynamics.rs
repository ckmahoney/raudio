...
pub fn apply_attack_release(
  current_env: f32, input: f32, attack_coeff: f32, release_coeff: f32, is_holding: bool,
) -> f32 {
  if is_holding {
    current_env
...
pub fn validate_gate_params(params: &GateParams) -> Result<(), String> {
  ...
  if params.threshold < MIN_DB || params.threshold > MAX_DB {
    return Err(format!("Threshold must be between {} dB and {} dB.", MIN_DB, MAX_DB));
  }
