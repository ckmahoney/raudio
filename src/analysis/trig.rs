...
pub fn merge_soids(amplitudes: &Vec<f32>, phases: &Vec<f32>) -> (f32, f32) {
  let real_sum: f32 = amplitudes.iter().zip(phases.iter()).map(|(&amp, &phi)| amp * phi.cos()).sum();

  let imag_sum: f32 = amplitudes.iter().zip(phases.iter()).map(|(&amp, &phi)| amp * phi.sin()).sum();
